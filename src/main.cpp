...
#if defined(NDEBUG)
# error "Doubloons cannot be compiled without assertions."
#endif
...
{
    CAmount nSubsidy = 6.77 * COIN; //DBL
    int halvings = nHeight / Params().SubsidyHalvingInterval();

    // Force block reward to zero when right shift is undefined.
    if (halvings >= 1) //DBL - no more reward YARrrrr :P
        return nFees;
...
void ThreadScriptCheck() {
    RenameThread("doubloons-scriptch");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Doubloons: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Doubloons: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
